...
from models import db, Candidate, Job, JobCandidateMatch, CandidateRating, Recruiter
...
def add_sample_candidates():
    ...
    with application.app_context():
        ...
        if not jobs:
            print("Warning: No active jobs found. Cannot create job matches.")
        ...
        for candidate_data in sample_candidates:
            ...
            # Create matches for each job
            for job in jobs:
                match_score = round(candidate_data['match_score'] * (0.8 + (hash(job.title) % 40) / 100.0), 2)  # Varies score slightly per job
        ...
        print("\nAll sample candidates added successfully!")
